/**
 * A other proposal for (json/object)-query which :
...
define(["require", "../index", "./rql", "deep-utils/lib/array", "deep-utils/lib/catch-parenthesis"],
	function(require, nodes, rqler, arrayUtils, parenthesis) {
		...
		DQ.prototype.analyse = function dqAnalyse(path) {
			...
			while (rest.length > 0) {
				...
				if (paths.length === 0)
					throw new Error("deep-queries need at least one move path : " + path);
				if (rest.length === 0)
					break;
			...
			splitted.forEach(function(e) {
				if (e === "") {
					...
					if (rest.length === 0 || rest[0] != "-")
						throw new Error("DeepQueryError : when you use @.length : you could only use minus '-' operator followed by an integer. " + path + " - " + parts.join(","));
					...
					if (isNaN(integs))
						throw new Error("DeepQueryError : bad index : index unknown. " + path + " - " + parts.join(","));
					value = function(parent) {
						return integs;
					};
				}
		...
		DQ.prototype.analyseUnionAccess = function dqanalyseUnionAccess(path, parts) {
			if (path[0] != '[')
				throw new Error("DeepQueryError : union access need to start with '['. " + path + " - " + parts.join(","));
			var inner = "";
			var othis = this;
			var count = 1;
			while (path[count] != ']' && count < path.length)
		...
		DQ.prototype.analyseSelector = function(path, parts, fromUnion) {
			...
			if (path[0] == "?") {
				...
				if (fromUnion)
					throw new Error("you couldn't write '?' in union of selectors. " + path + " - " + parts.join(","));
			...
			if (path[0] == '[') {
				if (fromUnion)
					throw new Error("DeepQueryError :  you couldn't have union in union of selectors. " + this.currentQuery);
		...
		DQ.prototype.analyseMoves = function(path, paths) {
			...
			while (a < path.length && (path[a] == "." || path[a] == "/")) {
				...
				if (tmp.length > 0) {
					if (tmp.length > 2)
						throw new Error("DeepQueryError : bad move : " + path);
					steps.push(tmp);
					tmp = '';
				}
				while (path[a] == '.') {
			...
			if (!last)
				throw new Error("DeepQueryError : missformed query : " + this.asked);
			if (last[0] == ".")
				a -= last.length;
			...
			while (steps.length > 0) {
				...
				if (step != '/' && step != '//')
					throw new Error("DeepQueryError : bad move : " + JSON.stringify(path) + " - " + step);
				res.slashes = step;
				paths.push(res);
		...
		DQ.prototype.doMove = function(move, items) {
			var newItems = [];
			var toDo = move;
			var self = this;
				...
				if (toDo.points)
					switch (toDo.points) {
						...
						case "...":
							...
							break;
						default:
							throw new Error("DeepQueryError : bad move : " + toDo);
					} else
						newItems.push(item);
				if (toDo.slashes == "//")
		...
		DQ.prototype.query = function(obj, q, options) {
			...
			if (parts.length === 0 || parts[0].type != "move")
				throw new Error("DeepQueryError : query need to start with move : " + q);
			var self = this,
				start = true,
				count = 1,
				part = parts[0];
			...
			if (s._deep_array_) {
				...
				if (!root) {
					...
					s.forEach(function(n) {
						r = r.concat(DQ.query(n, q, opt));
					});
					if (r.length)
						r = arrayUtils.arrayUnique(r, "path");
					r._deep_array_ = true;
					return r;
				}
			}
